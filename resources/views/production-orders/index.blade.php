    ...
    <div class="overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm ring-1 ring-gray-900/5">
        <div class="flex flex-col gap-4 border-b border-gray-100 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-base font-semibold text-gray-900">{{ __('app.nav_production_orders') }}</h2>
                    <p class="text-sm text-gray-500">{{ __('app.production_orders_intro') }}</p>
                </div>
            </div>
            <a href="{{ route('production-orders.create') }}" class="inline-flex items-center justify-center gap-1.5 rounded-lg bg-[#2b2361] px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#231c4d]">
            ...
            <table class="min-w-full text-sm">
                <thead>
                    <tr class="border-b border-gray-100 bg-gray-50/60">
                        ...
                        <th class="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('app.field_product') }}</th>
                        <th class="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('app.field_quantity') }}</th>
                        <th class="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('app.field_status') }}</th>
                        <th class="px-6 py-3 text-start text-xs font-semibold uppercase tracking-wider text-gray-500">{{ __('app.field_start_date') }}</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($orders as $order)
                        <tr class="transition-colors hover:bg-gray-50/80">
                            ...
                            <td class="px-6 py-4 text-gray-500">{{ $order->quantity }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-gray-500">{{ $order->start_date?->format('Y-m-d') ?? '—' }}</td>
                            <td class="px-6 py-4 text-end">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('production-orders.edit', $order) }}" title="{{ __('app.edit') }}" class="rounded-lg p-2 text-gray-400 transition hover:bg-violet-50 hover:text-violet-600">
                                    ...
                                    <form method="POST" action="{{ route('production-orders.destroy', $order) }}" onsubmit="return confirm('{{ __('app.confirm_delete') }}')">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-16">
                                <div class="flex flex-col items-center gap-3 text-gray-400">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82"></path></svg>
