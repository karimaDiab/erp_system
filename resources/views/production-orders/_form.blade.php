...
<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
    <div>
        ...
        <select name="product_id" required class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-[#3A306F] focus:ring-[#3A306F]">
            <option value="">—</option>
                ...
                <tr class="item-row">
                    <td class="px-3 py-2">
                        <select name="materials[{{ $i }}][product_id]" class="w-full rounded-lg border border-gray-300 px-2 py-1.5 text-sm" required>
                            <option value="">—</option>
                            @foreach ($products as $product)
                                <option value="{{ $product->id }}" @selected($material->product_id == $product->id)>{{ $product->name }} ({{ $product->sku }})</option>
                            @endforeach
                        </select>
                    </td>
                    <td class="px-3 py-2">
