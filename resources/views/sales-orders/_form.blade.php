...
<div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
    <div>
        <label class="mb-1.5 block text-sm font-medium text-gray-700">{{ __('app.field_customer') }}</label>
        <select name="customer_id" required class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-[#3A306F] focus:ring-[#3A306F]">
            <option value="">—</option>
            @foreach ($customers as $customer)
                <option value="{{ $customer->id }}" @selected(old('customer_id', $order->customer_id ?? null) == $customer->id)>{{ $customer->name }}</option>
            @endforeach
        </select>
    ...
    <div>
        ...
        <select name="status" required class="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-[#3A306F] focus:ring-[#3A306F]">
            @foreach (\App\Models\SalesOrder::STATUSES as $status)
...
<div class="mt-8 flex gap-3">
    ...
    <a href="{{ route('sales-orders.index') }}" class="rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-50">{{ __('app.cancel') }}</a>
</div>
