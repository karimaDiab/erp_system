<x-auth-layout :heading="__('auth.create_account')" :subheading="__('auth.register_subtitle')">
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        @csrf

        <div>
            <label for="name" class="mb-1.5 block text-sm font-medium text-white/80">{{ __('auth.full_name') }}</label>
            <input
                id="name"
                type="text"
                name="name"
    ...
    <p class="mt-6 text-center text-sm text-white/60">
        ...
        <a href="{{ route('login') }}" class="font-semibold text-white hover:underline">{{ __('auth.sign_in') }}</a>
    </p>
</x-auth-layout>
