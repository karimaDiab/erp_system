<x-app-layout :title="__('app.nav_purchase_orders')">
    <div class="mb-6 flex items-center justify-between">
        ...
        <a href="{{ route('purchase-orders.create') }}" class="rounded-lg bg-[#3A306F] px-4 py-2 text-sm font-semibold text-white hover:bg-[#2f2759]">
            + {{ __('app.add_new') }}
        </a>
    </div>
                ...
                @forelse ($orders as $order)
                    <tr>
                        ...
                        <td class="px-6 py-3 text-end">
                            <a href="{{ route('purchase-orders.edit', $order) }}" class="font-medium text-[#3A306F] hover:underline">{{ __('app.edit') }}</a>
